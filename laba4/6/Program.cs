...
class prog
{
    static void Main(string[] args)
    {
        ...
        int[] samCoordinates = Initialize(matrix);
        ...
        foreach (var move in command)
        {
            UpdateE(matrix);
            CheckE(matrix);
            MoveS(move, matrix, samCoordinates);
            ChekN(matrix);
        }
    }

    private static void ChekN(char[][] matrix)
    {
    ...
    private static void MoveS(char move, char[][] matrix, int[] coordinates)
    {
        switch (move)
        {
            case 'U':
    ...
    private static void CheckE(char[][] matrix)
    {
        for (var line = 0; line < matrix.Length; line++)
        {
